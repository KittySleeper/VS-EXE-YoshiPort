...
var stateShit:Array<FlxState> = [
	new StoryMenuState(),
	new ModState("EncoreState"),
	new ModState("FreeplayState"),
	new ModState("SoundTestState"),
	new options.screens.OptionMain(),
	new ModState("ExtrasState")
];

var theBestEverIndex = 0;
...
function update(elapsed) {
	...
	var justFaggot = FlxG.keys.justPressed;
	var theBestEver = [
		justFaggot.S,
		justFaggot.E,
		justFaggot.R,
	...
	for (key in theBestEver) {
		stupidAss++;

		if (key && stupidAss == theBestEverIndex) {
			theBestEverIndex++;

			if (theBestEverIndex == theBestEver.length) {
                ...
                new FlxTimer().start(1, function(tmr) {
                    CoolUtil.loadSong(mod, "Personel", "hard");
